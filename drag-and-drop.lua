...
local function generateSquares()
		...
		for i = 1, numSquares do
			...
			squares[i].customBounds = {xMin = squares[i].x - squares[i].width/2, xMax = squares[i].x + squares[i].width/2, yMin = squares[i].y - squares[i].height/2, yMax = squares[i].y + squares[i].height/2}
		end
	return squares
end
...
local function swapSquares(s1, s2)
	...
	for i = 1, #squares do
		...
		if isDrop and i >= s2 and i < s1 then
			...
			squares[i].customBounds = {xMin = squares[i].x - squares[i].width/2, xMax = squares[i].x + squares[i].width/2, yMin = areaBounds.yMin + padding + (nearestFreeIndex - 1) * (squareHeight + padding), yMax = areaBounds.yMin + padding + (nearestFreeIndex - 1) * (squareHeight + padding) + squareHeight}
			else
				squares[i].customBounds = {xMin = squares[i].x - squares[i].width/2, xMax = squares[i].x + squares[i].width/2, yMin = targetY - squares[i].height/2, yMax = targetY + squares[i].height/2}
			end
		end
end
